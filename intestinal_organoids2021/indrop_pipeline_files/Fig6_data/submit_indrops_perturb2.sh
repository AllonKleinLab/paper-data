...
logdir="logs_perturb2_rc/" # path for writing log files
yaml="20200904_perturb_rc.yaml"
...
## PART 4 (quantify)
jid4=$( sbatch --dependency=afterany:$jid3 -p short --array 1-${N4} --job-name Q --mem 3000 -t 06:00:00 -o "$logdir"quant_worker_%a.out -e "$logdir"quant_worker_%a.out --wrap """ ${myPython} $indrops $yaml quantify --min-reads 250 --min-counts 0 --total-workers ${N4} --worker-index \$((\$SLURM_ARRAY_TASK_ID-1)) --no-bam """)
jid4=${jid4##* }
echo $jid4

## PART 5 (aggregate)
# max number of workers = number of libraries
jid5=$( sbatch --dependency=afterany:$jid4 -p short --job-name A --mem 16000 -t 06:00:00 -o "$logdir"agg.out -e "$logdir"agg.out --wrap """ ${myPython} $indrops $yaml aggregate --no-bam """ )
jid5=${jid5##* }
echo $jid5