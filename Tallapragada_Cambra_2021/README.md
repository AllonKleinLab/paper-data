...
6. Remove any excess air and add the needle to the syringe. You can keep the cap or alternatively, use micro tubing which you can then tie and tape the end of to prevent air from coming into contact with the solution. The latter is how we normally store our Aquapel, but this is because it has multiple uses in our lab. If you need to use Aquapel for this purpose only, feel free to use whatever glass container is useful for you; it's just critical that the Aquapel is not stored in plastic and is kept from exposure to the air. <br>
