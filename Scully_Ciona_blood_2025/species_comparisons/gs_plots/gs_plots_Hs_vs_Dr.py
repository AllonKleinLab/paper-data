...
sc.tl.rank_genes_groups(adata_dr, groupby='cell_type', method='wilcoxon')
degs_dr = {}
for cell_type in adata_dr.obs['cell_type'].cat.categories:
    df = sc.get.rank_genes_groups_df(adata_dr, cell_type)
    these_genes = set(df['names'][(df['logfoldchanges'] > 2)
                                  * (df['pvals_adj'] < 0.05)])
    degs_dr[cell_type] = these_genes
...
# ============================================================================
# GENE HOMOLOGY FOR HUMAN VS. ZEBRAFISH

dr_gene_list = adata_dr.var.index
...
orthofinder_all = []
for dr_gene in dr2hs:
    for hs_gene in dr2hs[dr_gene]:
        if (dr_gene in dr_gene_list) and (hs_gene in hs_gene_list):
            orthofinder_all.append([dr_gene, hs_gene, 1])
...
jaccard_hs = pd.DataFrame(
    ...
    columns=[f'Dr_{x}' for x in adata_dr.obs['cell_type'].cat.categories]
)
...
def plot_for_hs_cell(hs_cell, fig_width=2.5, ortholog='orthofinder',
    ...
    # --------------------------------
    # Get zebrafish cell types to plot
    dr_cell_list = set(jaccard.columns)
    dr_cell_list = [x.replace('Dr_', '') for x in dr_cell_list]

    # Sort Ciona cell types by all-gene similarity
    order = np.argsort(jaccard.loc['Hs_' + hs_cell,
                                       ['Dr_'+x for x in dr_cell_list]])[::-1]
    dr_cell_list = list(np.array(dr_cell_list)[order.values])

    # For zfish, keep all cell types
    # Keep the top n
    dr_cell_list = dr_cell_list[:n]

    if len(dr_cell_list) == 0:
        print(f'{hs_cell}: No similar cell types')
        return None

    # --------------------------------
    # Get numbers for plot
    hs_genes, dr_genes = get_gene_dict_sets(
        hs_cell, dr_cell_list, gene_orthology=ortholog)
    hs_rects_dict, dr_rects_dict, ribbon_widths = get_sankey_plot_numbers(
        hs_genes, dr_genes)
    hs_rects_unmatched = {x: len(degs_hs[x]) - hs_rects_dict[x]
                          for x in hs_rects_dict}
    dr_rects_unmatched = {x: len(degs_dr[x]) - dr_rects_dict[x]
                          for x in dr_rects_dict}
    
    
    # Filter to only include cell types with >N genes
    dr_rects_dict = {x: dr_rects_dict[x] for x in dr_rects_dict
                     if (dr_rects_dict[x] > 10) and (x in [y[1] for y in ribbon_widths])}
    ribbon_widths = [x for x in ribbon_widths if x[1] in dr_rects_dict]

    # Rename cell types for plotting
    hs_rects_dict = {x.replace(' ', '\n'): hs_rects_dict[x] for x in hs_rects_dict}
    dr_rects_dict = {x.replace(' ', '\n'): dr_rects_dict[x] for x in dr_rects_dict}
    ...
    plot_modified_sankey(hs_rects_dict, dr_rects_dict, ribbon_widths,
                         ...
                         bot_rect_unmatched=dr_rects_unmatched,
...
plot_for_hs_cell('hematopoietic progenitor', fig_width=2.5, ortholog='orthofinder', out_path=out_path2)
plot_for_hs_cell('neutrophil', fig_width=2.5, ortholog='orthofinder', out_path=out_path2)
plot_for_hs_cell('monocyte', fig_width=2.5, ortholog='orthofinder', out_path=out_path2)
plot_for_hs_cell('macrophage', fig_width=2.5, ortholog='orthofinder', out_path=out_path2)
plot_for_hs_cell('NK cell', fig_width=2.5, ortholog='orthofinder', out_path=out_path2)
plot_for_hs_cell('erythrocyte', fig_width=2.5, ortholog='orthofinder', out_path=out_path2)
plot_for_hs_cell('T cell', fig_width=2.5, ortholog='orthofinder', out_path=out_path2)
...
def plot_for_dr_cell(dr_cell, fig_width=2.5, ortholog='orthofinder',
                     out_path=out_path):
    """
    ...
    # Get numbers for plot
    hs_genes, dr_genes = get_gene_dict_sets(
        dr_cell, hs_cell_list, sp1='zfish',
        gene_orthology=ortholog)
    dr_rects_dict, hs_rects_dict, ribbon_widths = get_sankey_plot_numbers(
        dr_genes, hs_genes)
    hs_rects_unmatched = {x: len(degs_hs[x]) - hs_rects_dict[x]
                          for x in hs_rects_dict}
    dr_rects_unmatched = {x: len(degs_dr[x]) - dr_rects_dict[x]
    ...
    # Plot
    plot_modified_sankey(dr_rects_dict, hs_rects_dict, ribbon_widths,
                         top_rect_unmatched=dr_rects_unmatched,
                         bot_rect_unmatched=hs_rects_unmatched,
                         rect_height=0.15, vertical_rect_spacing=1,
                         scalebar_size=100, figsize=(fig_width, fig_height),
                         color_top=sp1_color, color_bot=sp2_color)
    plt.savefig(os.path.join(out_path, f'{dr_cell.replace("/","-")}.pdf'))
    plt.close()


# ------------------------------------
# Make and save plots
out_path2 = os.path.join(out_path, 'plots_zfish_top')
if not os.path.exists(out_path2): os.mkdir(out_path2)
plot_for_dr_cell('neutrophil', fig_width=3, ortholog='orthofinder', out_path=out_path2)
plot_for_dr_cell('macrophage', fig_width=3, ortholog='orthofinder', out_path=out_path2)
plot_for_dr_cell('HPC', fig_width=3, ortholog='orthofinder', out_path=out_path2)
...
sns.heatmap(
    jaccard.loc[sp2_cell_type_order, sp1_cell_type_order],
    cmap='rocket_r',
    xticklabels=True,
    yticklabels=True,
    vmin=0, vmax=0.12,
...
ax.set_xticklabels([x.split('_')[-1] for x in sp1_cell_type_order], rotation=90)
ax.set_yticklabels([x.split('_')[-1] for x in sp2_cell_type_order])
plt.tight_layout()
