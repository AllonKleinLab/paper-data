...
# batch = 'library'
# version = 'harmony_v1'
# data_path = hf.path.sc_analysis + '000_processing/batch_correction_OLD_output/'

version = 'library_no_doublet_cluster'
int_method = 'bbknn'
data_path = (hf.path.sc_analysis
             + f'000_processing/batch_correction_output/{version}/')

# Import counts matrix
adata = sc.read_h5ad(data_path + f'adata_{int_method}_annotated.h5ad')

# Set up output folder
out_path = 'go_enrichment_analysis_output/'
if not os.path.exists(out_path): os.mkdir(out_path)

out_path += f'{version}_{version}/'
if not os.path.exists(out_path): os.mkdir(out_path)

# ============================================================================
# GO term gene sets with Ciona genes

# go_gene_sets = 'GO_Molecular_Function_2023'
# go_gene_sets = 'GO_Biological_Process_2023'
go_gene_sets = 'GO_Cellular_Component_2023'

out_path += f'{go_gene_sets}/'
if not os.path.exists(out_path): os.mkdir(out_path)
...
try:
    ...
    fisher_results[fisher_results['null rejected']].to_csv(
        out_path + 'fisher_results.tsv', sep='\t'
    )
    # fisher_results.to_csv(out_path + 'fisher_results.tsv', sep='\t')
