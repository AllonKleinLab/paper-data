...
jid2=$( sbatch --dependency=afterany:$jid1 -p short --job-name I --mem 16000 -t 3:00:00 -o "$logdir"abundant.out -e "$logdir"abundant.out --wrap """ ${myPython} $indrops $yaml identify_abundant_barcodes """ )
jid2=${jid2##* }
echo $jid2
...
## PART 4 (quantify)
jid4=$( sbatch --dependency=afterany:$jid3 -p short --array 1-${N4} --job-name Q --mem 3000 -t 12:00:00 -o "$logdir"quant_worker_%a.out -e "$logdir"quant_worker_%a.out --wrap """ ${myPython} $indrops $yaml quantify --min-reads 250 --min-counts 0 --total-workers ${N4} --worker-index \$((\$SLURM_ARRAY_TASK_ID-1)) --no-bam """)
jid4=${jid4##* }
echo $jid4

## PART 5 (aggregate)
# max number of workers = number of libraries
jid5=$( sbatch --dependency=afterany:$jid4 -p short --job-name A --mem 8000 -t 08:00:00 -o "$logdir"agg.out -e "$logdir"agg.out --wrap """ ${myPython} $indrops $yaml aggregate --no-bam """ )
jid5=${jid5##* }
echo $jid5